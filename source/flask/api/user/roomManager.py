...
def searchRoomDetail(data):
    ...
    result["DATA"] = [
        {"ROOM_NAME": ROOM_NAME, "ROOM_UUID": ROOM_UUID}
        for (ROOM_NAME, ROOM_UUID) in cursor
    ]
    cursor.close()
    connection.close()
    return result
...
def getRoomData(req, admin):
    ...
    result["DATA"] = [
        {
            ...
            "OCCUPANCY": OCCUPANCY,
            "LAST_DATA_TS": (
                LAST_DATA_TS.astimezone(local_timezone).astimezone(pytz.utc)
                if LAST_DATA_TS
                else LAST_DATA_TS
            ),
            "LAST_DATA_RECEIVED": (
    ...
    for room in result["DATA"]:
        cursor.execute(
            f"""SELECT `ID`,`TIMESTAMP`,`URGENCY`,`TYPE`,`DETAILS`,`NOTIFY` ,`NOTIFY_TIMESTAMP`, ACCURACY FROM `ALERT` WHERE `ROOM_ID`={room["ID"]} AND `NOTIFY`=0 GROUP BY `URGENCY` ORDER BY `URGENCY` DESC;"""
        )
        room["ALERTS"] = [
            {
                "ID": ID,
                "TIMESTAMP": (
                    TIMESTAMP.astimezone(local_timezone).astimezone(pytz.utc)
                    if TIMESTAMP
                    else TIMESTAMP
                ),
                "URGENCY": URGENCY,
                ...
                "NOTIFY": NOTIFY,
                "NOTIFY_TIMESTAMP": (
                    NOTIFY_TIMESTAMP.astimezone(local_timezone).astimezone(pytz.utc)
                    if NOTIFY_TIMESTAMP
                    else NOTIFY_TIMESTAMP
                ),
                "ACCURACY": ACCURACY,
            }
            for (
...
def getSpecificRoomData(req, admin):
    ...
    # sql = "SELECT * FROM ROOMS_DETAILS WHERE MAC='%s'"%(req['MAC'])
    sql = (
        "SELECT ID, ROOM_LOC, ROOM_NAME, ROOM_UUID, IMAGE_NAME, INFO, ROOM_X, ROOM_Y FROM Gaitmetrics.ROOMS_DETAILS WHERE ROOM_UUID='%s'"
        % (req["ROOM_UUID"])
    )
    ...
    if dbresult:
        for x in dbresult:
            if x[0] == UID:
                UID = uuid.uuid4().hex
    cursor.execute(
        "INSERT INTO Gaitmetrics.ROOMS_DETAILS (ROOM_LOC, ROOM_NAME, ROOM_UUID, IMAGE_NAME, INFO, ROOM_X, ROOM_Y) VALUES(%s, %s, %s, %s, %s, %s, %s)",
        (
            data["ROOM_LOC"],
            data["ROOM_NAME"],
            UID,
...
def updateRoomDetail(data, uploadsLoc):
    ...
    cursor = connection.cursor()
    sql = (
        "UPDATE Gaitmetrics.ROOMS_DETAILS SET ROOM_LOC='%s', ROOM_NAME='%s', IMAGE_NAME='%s', INFO='%s', ROOM_X='%s', ROOM_Y='%s' WHERE ROOM_UUID='%s'"
        % (
            data["ROOM_LOC"],
            data["ROOM_NAME"],
            data["IMAGE_NAME"],
...
def deleteRoomDetail(data, uploadsLoc):
    ...
    if not dbresult:
        ...
        return result
    sql = "DELETE FROM Gaitmetrics.ROOMS_DETAILS WHERE ROOM_UUID='%s'" % (
        data["ROOM_UUID"]
    )
    try:
...
def getRoomAlertsData(room_uuid, unread=True, set=True):
    ...
    sql = (
        f"SELECT a.`ID`,a.`TIMESTAMP`,a.`URGENCY`,a.`TYPE`,a.`DETAILS`,a.`NOTIFY` ,a.`NOTIFY_TIMESTAMP` , a.`ACCURACY`FROM `ALERT` a LEFT JOIN `ROOMS_DETAILS` r ON a.`ROOM_ID`=r.`ID` WHERE r.`ROOM_UUID`='{room_uuid}'"
        + option
    )
    cursor.execute(sql)
    local_timezone = get_localzone()
        ...
        result["DATA"].append(
            {
                ...
                "NOTIFY": notify,
                "NOTIFY_TIMESTAMP": (
                    notify_ts.astimezone(local_timezone).astimezone(pytz.utc)
                    if notify_ts
                    else notify_ts
                ),
                "ACCURACY": accuracy,
...
def markAlertsAccuracy(alert_id, accuracy):
    try:
        connection = mysql.connector.connect(**config)
        cursor = connection.cursor()
        sql = f"UPDATE `ALERT` a SET a.`ACCURACY`={accuracy} WHERE a.`ID`='{alert_id}'"
        cursor.execute(sql)
        connection.commit()
        cursor.close()
        connection.close()
...
def getRoomsAlerts(MAC, unread=True):
    ...
    sql = (
        f"SELECT a.`ID`,a.`TIMESTAMP`,a.`URGENCY`,a.`TYPE`,a.`DETAILS`,a.`NOTIFY` ,a.`NOTIFY_TIMESTAMP` FROM `ALERT` a LEFT JOIN `ROOMS_DETAILS` r ON a.`ROOM_ID`=r.`ID` LEFT JOIN `RL_ROOM_MAC` rm ON r.`ROOM_UUID`=rm.`ROOM_UUID` WHERE a.URGENCY=3 AND rm.`MAC`='{MAC}'"
        + option
    )
    cursor.execute(sql)
    local_timezone = get_localzone()
...
def getFilterLocationHistoryData(room_id):
    ...
    result["DATA"] = [
        ...
        for (X_START, X_END, Y_START, Y_END) in cursor
    ]
