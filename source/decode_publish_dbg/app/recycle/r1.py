...
# mqttc.will_set("/GMT/USVC/DECODE_PUBLISH/STATUS","DISCONNECTED",1,True)
...
def decode_process_publish(mac, data):
    ...
    for ts_str, byteAD in data.items():
        ...
        if len(byteAD) > 52:
            ...
            if radarType == 'wall':
                ...
                if 'numDetectedTracks' in outputDict and len(wallStateParam[mac]['previous_pointClouds']) > 0 and \
                ...
                # if 'state' in wall_Dict:
                    # print(wall_Dict['state'])
                # if 'kidOrAdult' in wall_Dict:
                #     print(wall_Dict['kidOrAdult'])
    ...
    if len(pubPayload["DATA"]) > 0:
        ...
        print("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")       
        if radarType == 'wall':
            mqttc.publish("/GMT/DEV/"+mac+"/DATA/WALL/JSON1",jsonData)
        elif radarType == 'ceil':
            mqttc.publish("/GMT/DEV/"+mac+"/DATA/CEIL/JSON1",jsonData)
        elif radarType == 'vital':
            mqttc.publish("/GMT/DEV/"+mac+"/DATA/VITAL/JSON1",jsonData)
        time.sleep(0.1)
...
# mqttc.publish("/GMT/USVC/DECODE_PUBLISH/STATUS","CONNECTED",1, True)
time.sleep(1)
