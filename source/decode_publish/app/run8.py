...
# Decode, Process, and Publish MQTT Data Packets from Radar
# def decode_process_publish(mac, data, radarType, xShift, yShift, zShift, rotXDegree, rotYDegree, rotZDegree, aggregate_period):
def decode_multiProcess_publish(mqttc, stateParam_sharedDict, algoCfg_sharedDict, devicesTblQueue, dataBufferQueue):
    # global mqttc, config, aggregate_period, devicesTbl, breathRate_MA, heartRate_MA, algoCfg
  while 1:
    while devicesTblQueue.empty():
        time.sleep(0.1)
    devicesTbl = devicesTblQueue.get()
    algoCfg = {"DATA":algoCfg_sharedDict["DATA"]}
    mac = devicesTbl.keys()[0]
...
def on_message3(mosq, obj, msg):
    global devicesTbl,config,aggregate_period,algoCfg, poolStart
    ...
    #     print("Error Processing")
        if poolStart == 0:
            pool1.apply_async(decode_multiProcess_publish(mqttc, stateParam_sharedDict, algoCfg_sharedDict, devicesTblQueue, dataBufferQueue))
            poolStart == 1
        devicesTblQueue.put({devName:devicesTbl[devName]})
        # print(devName)
        devicesTbl[devName]["DATA_QUEUE"]={}
...
if __name__ == '__main__':
    ...
    pool1 = Pool(processes=multiprocessing.cpu_count()-2)
    manager = Manager()
    stateParam_sharedDict = manager.dict()
    algoCfg_sharedDict = manager.dict()
    devicesTblQueue = Queue()
