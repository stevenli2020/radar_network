...
def getregisterDeviceLists(req, admin):
    ...
    if admin:
        sql = "SELECT DEVICES.Id, DEVICES.MAC, NAME, TYPE, DEVICES.STATUS, DEPLOY_X, DEPLOY_Y, DEPLOY_Z, ROT_X, ROT_Y, ROT_Z, LAST_DATA_RECEIVED, DESCRIPTION, ROOMS_DETAILS.ROOM_NAME, DEVICES.CONFIG_SWITCH FROM Gaitmetrics.DEVICES LEFT JOIN Gaitmetrics.RL_ROOM_MAC ON Gaitmetrics.DEVICES.MAC=Gaitmetrics.RL_ROOM_MAC.MAC LEFT JOIN Gaitmetrics.ROOMS_DETAILS ON Gaitmetrics.RL_ROOM_MAC.ROOM_UUID=Gaitmetrics.ROOMS_DETAILS.ROOM_UUID"
        cursor.execute(sql)
        result["DATA"] = [{"Id": Id, "MAC": MAC, "NAME": NAME, "TYPE": TYPE, "STATUS":STATUS, "DEPLOY_X":DEPLOY_X, "DEPLOY_Y": DEPLOY_Y, "DEPLOY_Z":DEPLOY_Z, "ROT_X":ROT_X, "ROT_Y":ROT_Y, "ROT_Z":ROT_Z, "LAST DATA": LAST_DATA_RECEIVED.astimezone(local_timezone).astimezone(pytz.utc) if LAST_DATA_RECEIVED else LAST_DATA_RECEIVED, "DESCRIPTION": DESCRIPTION, "ROOM_NAME":ROOM_NAME, "CONFIG_SWITCH":CONFIG_SWITCH} for (Id, MAC, NAME, TYPE, STATUS, DEPLOY_X, DEPLOY_Y, DEPLOY_Z, ROT_X, ROT_Y, ROT_Z, LAST_DATA_RECEIVED, DESCRIPTION, ROOM_NAME, CONFIG_SWITCH) in cursor]
    else:
