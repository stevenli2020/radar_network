...
def decode_process_publish(mac, data):
    ...
    if len(pubPayload["DATA"]) > 0:
        ...
        if radarType == 'wall':
            ...
            mqttc.publish("/GMT/DEV/"+mac+"/DATA/WALL/JSON_DBG2", jsonData)
        elif radarType == 'ceil':
            pubPayload["TYPE"]="CEIL"
            jsonData = json.dumps(pubPayload)        
            mqttc.publish("/GMT/DEV/" + mac + "/DATA/CEIL/JSON_DBG2", jsonData)
        elif radarType == 'vital':
            pubPayload["TYPE"]="VITAL"
            jsonData = json.dumps(pubPayload)        
            mqttc.publish("/GMT/DEV/" + mac + "/DATA/VITAL/JSON_DBG2", jsonData)
        # print(jsonData)
        # print("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")              
        time.sleep(0.1)
