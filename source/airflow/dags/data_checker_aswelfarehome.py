...
def check_insufficient_data_by_sensor_type(data_type):
    ...
    for room in dbresult:
        ...
        for table in tables:
            try:
                sql = f"SELECT `TIMESTAMP`, 1 AS `WALL_DATA_COUNT` FROM Gaitmetrics.{table} WHERE MAC {List} AND `TIMESTAMP` >= NOW() - INTERVAL 240 MINUTE;"
                cursor.execute(sql)
...
with DAG(
    dag_id="data_checker_aswelfarehome",
    start_date=datetime(2024, 10, 12),
    schedule_interval="0 */4 * * *",
    catchup=False,
) as dag:

    check_servers_task = PythonOperator(
        task_id="CHECK_SERVERS",
        python_callable=check_servers,
