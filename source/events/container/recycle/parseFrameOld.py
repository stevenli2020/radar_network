...
def parseStandardFrame(frameData):
    ...
    tlvHeaderLength = 8
    # print("frameheader: ",frameHeaderLen)
    outputDict = {}
    outputDict['error'] = 0
    ...
    # outputDict['pointCloud'] = np.zeros((numDetectedObj, 7), np.float64) # error on this line
    ...
    for i in range(numTLVs):
        ...
        try:
            ...
            # print("tlvType ", tlvType)
            # print("tlvLength ", tlvLength)
            frameData = frameData[tlvHeaderLength:]
            # print("frameData upto headerLength: ", frameData)
            # with open("logs", "a+") as myfile:
            #     myfile.write("\n[tlvType];"+str(tlvType)+"\n[tlvLength]:"+str(tlvLength)+"\n[frameData up to tlvheaderlength]"+str(frameData[:tlvHeaderLength]))
            # tlvLength = tlvLength - tlvHeaderLength
            # if (tlvType == MMWDEMO_OUTPUT_MSG_RANGE_PROFILE):
            #     pass
            # # Noise Profile
