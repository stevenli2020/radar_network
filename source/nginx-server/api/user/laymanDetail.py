...
def getLaymanData(date, room_uuid):
    ...
    sql = (
        "SELECT CONCAT(ROOMS_DETAILS.ROOM_NAME,'@',ROOMS_DETAILS.ROOM_LOC) AS room_name, ROOMS_DETAILS.ID, GROUP_CONCAT(RL_ROOM_MAC.MAC) AS MAC FROM Gaitmetrics.ROOMS_DETAILS LEFT JOIN Gaitmetrics.RL_ROOM_MAC ON ROOMS_DETAILS.ROOM_UUID=RL_ROOM_MAC.ROOM_UUID WHERE ROOMS_DETAILS.ROOM_UUID='%s';"
        % (room_uuid)
    )
    cursor.execute(sql)
    ...
    if room_name:
        ...
        result["data"]["sensors"] = (
            room_name["MAC"].split(",") if room_name["MAC"] else []
        )
        ...
        sleeping_hour_max, sleeping_hour_min, sleeping_hour_average = (
            get_data_from_analysis(date, room_id, "sleeping_hour")
        )
        ...
        temp, temp, sleeping_hour_previous = get_data_from_analysis(
            previous_date, room_id, "sleeping_hour"
        )
        result["data"]["sleeping_hour"]["previous_average"] = sleeping_hour_previous

        bed_time_max, bed_time_min, bed_time_average = get_data_from_analysis(
            date, room_id, "bed_time"
        )
        result["data"]["bed_time"]["max"] = bed_time_max
        result["data"]["bed_time"]["min"] = bed_time_min
        result["data"]["bed_time"]["average"] = bed_time_average
        temp, temp, bed_time_previous = get_data_from_analysis(
            previous_date, room_id, "bed_time"
        )
        result["data"]["bed_time"]["previous_average"] = bed_time_previous

        wake_up_time_max, wake_up_time_min, wake_up_time_average = (
            get_data_from_analysis(date, room_id, "wake_up_time")
        )
        ...
        temp, temp, wake_up_time_previous = get_data_from_analysis(
            previous_date, room_id, "wake_up_time"
        )
        result["data"]["wake_up_time"]["previous_average"] = wake_up_time_previous

        time_in_bed_max, time_in_bed_min, time_in_bed_average = get_data_from_analysis(
            date, room_id, "time_in_bed"
        )
        ...
        temp, temp, time_in_bed_previous = get_data_from_analysis(
            previous_date, room_id, "time_in_bed"
        )
        result["data"]["time_in_bed"]["previous_average"] = time_in_bed_previous

        in_room_max, in_room_min, in_room_average = get_data_from_analysis(
            date, room_id, "in_room"
        )
        result["data"]["in_room"]["max"] = in_room_max
        result["data"]["in_room"]["min"] = in_room_min
        result["data"]["in_room"]["average"] = in_room_average
        temp, temp, in_room_previous = get_data_from_analysis(
            previous_date, room_id, "in_room"
        )
        result["data"]["in_room"]["previous_average"] = in_room_previous

        sleep_disruption_max, sleep_disruption_min, sleep_disruption_average = (
            get_data_from_analysis(date, room_id, "sleep_disruption")
        )
        ...
        temp, temp, sleep_disruption_previous = get_data_from_analysis(
            previous_date, room_id, "sleep_disruption"
        )
        result["data"]["sleep_disruption"][
            "previous_average"
        ] = sleep_disruption_previous

        breath_rate_max, breath_rate_min, breath_rate_average = get_data_from_analysis(
            date, room_id, "breath_rate"
        )
        ...
        temp, temp, breath_rate_previous = get_data_from_analysis(
            previous_date, room_id, "breath_rate"
        )
        result["data"]["breath_rate"]["previous_average"] = breath_rate_previous

        heart_rate_max, heart_rate_min, heart_rate_average = get_data_from_analysis(
            date, room_id, "heart_rate"
        )
        ...
        temp, temp, heart_rate_previous = get_data_from_analysis(
            previous_date, room_id, "heart_rate"
        )
        result["data"]["heart_rate"]["previous_average"] = heart_rate_previous

        disrupt_duration_max, disrupt_duration_min, disrupt_duration_average = (
            get_data_from_analysis(date, room_id, "disrupt_duration")
        )
        ...
        temp, temp, disrupt_duration_previous = get_data_from_analysis(
            previous_date, room_id, "disrupt_duration"
        )
        result["data"]["disrupt_duration"][
            "previous_average"
        ] = disrupt_duration_previous
...
def daily_inroom_analysis(date, room_id):
    ...
    if (moving + upright + laying + social) >= total:
        total = moving + upright + laying + social
...
def get_daily_data(date, room_id, type):
    ...
    try:
        sql = (
            "SELECT `VALUE` FROM `ANALYSIS_DAY` WHERE `TYPE`='%s' AND `ROOM_ID`=%s AND `DATE`='%s' UNION ALL SELECT '-' AS `VALUE` LIMIT 1;"
            % (type, room_id, date)
        )
        cursor.execute(sql)
...
def get_data_from_analysis(date, room_id, type):
    ...
    cursor = connection.cursor(dictionary=True)
    sql = (
        "SELECT max,min,average FROM `ANALYSIS` WHERE `TYPE`='%s' AND `ROOM_ID`=%s AND `EOW`='%s' UNION ALL SELECT '-' AS `max`,'-' AS `min`,'-' AS `average` LIMIT 1;"
        % (type, room_id, date)
    )
    cursor.execute(sql)
    data = cursor.fetchone()
