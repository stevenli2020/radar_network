...
# Decode tracker height
def parseTargetHeightTLV(tlvData, tlvLength):
    heightStruct = 'I2f'  # Track ID, max Z, min Z
    heightSize = struct.calcsize(heightStruct)
    numHeights = int(tlvLength / heightSize)
    heights = np.empty((numHeights,3))
    for i in range(numHeights):
        try:
            targetHeight = struct.unpack(heightStruct, tlvData[:heightSize])
        except:
            print('ERROR: Target Height TLV Parsing Failed')
            return heights

        heights[i, 0] = targetHeight[0] # Target ID
        heights[i, 1] = targetHeight[1] # max Z Position
        heights[i, 2] = targetHeight[2] # min Z Position
        tlvData = tlvData[heightSize:]
