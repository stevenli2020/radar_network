...
def get_profile(current_user):
    ...
    sql = f"SELECT ID, LOGIN_NAME, FULL_NAME, EMAIL, PHONE, TYPE, CREATED, LAST_UPDATE FROM Gaitmetrics.USERS WHERE ID={user_id}"
    cursor.execute(sql)
    result["DATA"] = [
        {
            "ID": ID,
            ...
            "TYPE": TYPE,
            "LAST_UPDATE": (
                LAST_UPDATE.astimezone(local_timezone).astimezone(pytz.utc)
                if LAST_UPDATE
                else LAST_UPDATE
            ),
            "CREATED": (
                CREATED.astimezone(local_timezone).astimezone(pytz.utc)
                if CREATED
                else CREATED
            ),
        }
...
def requestSpecificUser(data):
    ...
    local_timezone = get_localzone()
    sql = (
        "SELECT USERS.ID, LOGIN_NAME, FULL_NAME, EMAIL, PHONE, TYPE, USERS.STATUS, CODE, LAST_UPDATE, CREATED, GROUP_CONCAT(ROOMS_DETAILS.ROOM_NAME) AS ROOM_NAME FROM Gaitmetrics.USERS LEFT JOIN Gaitmetrics.RL_USER_ROOM ON USERS.ID=RL_USER_ROOM.USER_ID LEFT JOIN Gaitmetrics.ROOMS_DETAILS ON RL_USER_ROOM.ROOM_ID=ROOMS_DETAILS.ID WHERE USERS.ID='%s' GROUP BY USERS.ID"
        % (Id)
    )
    cursor.execute(sql)
    result["DATA"] = [
...
def addNewUser(data):
    ...
    now = datetime.now()
    cursor.execute(
        "INSERT INTO Gaitmetrics.USERS (LOGIN_NAME, FULL_NAME, EMAIL, PHONE, TYPE, STATUS, CODE, CREATED) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)",
        (
            data["LOGIN_NAME"],
            data["FULL_NAME"],
            data["EMAIL"],
        ...
        if dbresult:
            ...
            if data["ROOM"]:
                ...
                for Room in RoomList:
                    ...
                    print(RoomName)
                    sql = (
                        "SELECT * FROM Gaitmetrics.ROOMS_DETAILS WHERE ROOM_NAME='%s'"
                        % (RoomName)
                    )
                    cursor.execute(sql)
                    dbresult = cursor.fetchone()
                    ...
                    if dbresult:
                        cursor.execute(
                            "INSERT INTO Gaitmetrics.RL_USER_ROOM (USER_ID, ROOM_ID) VALUES(%s, %s)",
                            (id, dbresult[0]),
                        )
                        connection.commit()
                # for x in MAC:
    ...
    for i in range(1, 51):
        ...
        acl_data.append(
            (
                ...
                client_id,
                data["LOGIN_NAME"],
                "c764eb2b5fa2d259dc667e2b9e195218",
                '[{"pattern":"/GMT/#"}]',
                '[{"pattern":"/GMT/#"}]',
            )
        )
...
def deleteUserDetails(data):
    ...
    sql = "DELETE FROM vmq_auth_acl WHERE username='%s'" % (
        dbresult[1]
    )  # Assuming dbresult[1] contains the username
...
def getMQTTClientID(username):
    ...
    try:
        ...
        sql = (
            "SELECT client_id FROM vmq_auth_acl WHERE username='%s' AND connected=0 ORDER BY last_connect_time LIMIT 1"
            % (username)
        )  # Assuming dbresult[1] contains the username
        vernemq_db_cursor.execute(sql)
        ...
        if len(result) == 0:
            sql = (
                "SELECT client_id FROM vmq_auth_acl WHERE username='%s' AND connected=1 ORDER BY last_connect_time LIMIT 1"
                % (username)
            )  # Assuming dbresult[1] contains the username
            vernemq_db_cursor.execute(sql)
            result = vernemq_db_cursor.fetchall()
...
def setClientConnection(client_id):
    ...
    try:
        ...
        vernemq_db_cursor = vernemq_db_connection.cursor()
        sql = (
            "UPDATE vmq_auth_acl SET connected=1,last_connect_time=NOW() WHERE client_id='%s'"
            % (client_id)
        )  # Assuming dbresult[1] contains the username
        vernemq_db_cursor.execute(sql)
        vernemq_db_connection.commit()
