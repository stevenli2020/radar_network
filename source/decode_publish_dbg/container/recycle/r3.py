...
def cleanup():
    global mqttc
    mqttc.publish("/GMT/USVC/DECODE_PUBLISH_DBG3/STATUS","DISCONNECTED",1,True)
    print("\nDisconnecting...\n")
    mqttc.disconnect()
    time.sleep(1)
    print("Exit\n")
...
def decode_process_publish(mac, data):
        ...
        if len(byteAD) > 52:
            ...
            if radarType == 'wall':
                ...
                if wallStateParam[mac]['pandasDF'].empty:
                    ...
                    # aggregate_dict = wallStateParam[mac]['pandasDF'].agg({'timeStamp': 'mean',
                    #                                                'numSubjects': 'mean',
                    #                                                'roomOccupancy': 'mean',
                    ...
                    # if wallStateParam[mac]['pandasDF']['state'].isin([3]).sum() > 0:
                    #     aggregate_dict['state'] = 'Fall'
                    # elif aggregate_dict['state'].isin([4]).sum() == 0:
                    #     aggregate_dict['state'] = wallStateParam[mac]['label_state'][int(aggregate_dict['state'].iloc[0])]
                    # else:
                    #     aggregate_dict['state'] = None
                    ...
                    # else:
                        # print("ZZZZZZZZZZZZZ")
                        # aggregate_dict = {}
                        # aggregate_dict['timeStamp'] = wallStateParam[mac]['pandasDF']['timeStamp'].agg('mean')
                    ...
                    if aggregate_dict['kidOrAdult'] == 0:
                        aggregate_dict['kidOrAdult'] = 'Kid'
                    elif aggregate_dict['kidOrAdult'] == 1:
                        aggregate_dict['kidOrAdult'] = 'Adult'
            ...
            # ------------------------------------------------------------------------------------------------------------------
                ...
                if outputDict is not None:
                  if "numDetectedTracks" in outputDict and len(ceilStateParam[mac]['previous_pointClouds']) > 0 and \
                        "trackIndexes" in outputDict:
                    numTracks = outputDict['numDetectedTracks']
                    # pointClouds = outputDict['pointCloud']
                    trackIndices = outputDict['trackIndexes']
                    # trackUnique = np.unique(trackIndices)
                                ...
                                if len(ceilStateParam[mac]['averageX'][minDistIdx]) > 10:
                                    ...
                                    # elif len(x_coord[trackIndices == trackId]) > 10:
                                        ...
                                        if deltaDisp > 0.1 and len(x_coord[trackIndices == trackId]) > 10:
                                            ...
                                            elif body_width > 0.5 and y_height < 1.0 and ((body_width) / (y_height + 0.2)) > 2.0:
                                                # print("Laying")
                ...
                if ceilStateParam[mac]['pandasDF'].empty:
                    ...
                    if aggregate_dict:
                        ...
                        aggregate_dict = {}
                        aggregate_dict['timeStamp'] = round(ceilStateParam[mac]['pandasDF']['timeStamp'].agg('mean'),2)
                        aggregate_dict['numSubjects'] = 0
                        aggregate_dict['roomOccupancy'] = False
            ...
            # ------------------------------------------------------------------------------------------------------------------
                ...
                # numTracks = 0
                # print("============================\n", outputDict,"\n------------------------------\n",vitalStateParam, "\n==============================")
                if "numDetectedTracks" in outputDict and len(vitalStateParam[mac]['previous_pointClouds']) > 0 and "trackIndexes" in outputDict:
                    # print("+++++++++++++++++++++")
                    numTracks = outputDict['numDetectedTracks']
                    # pointClouds = outputDict['pointCloud']
                    trackIndices = outputDict['trackIndexes']
                ...
                if vitalStateParam[mac]['pandasDF'].empty:
                    ...
                    if str(aggregate_dict['bedOccupancy']) != 'nan':
                        ...
                        aggregate_dict = {}
                        aggregate_dict['timeStamp'] = round(vitalStateParam[mac]['pandasDF']['timeStamp'].agg('mean'),2)
                        aggregate_dict['numSubjects'] = 0
    ...
    if len(pubPayload["DATA"]) > 0:
        ...
        if radarType == 'wall':
            ...
            mqttc.publish("/GMT/DEV/"+mac+"/DATA/WALL/JSON3", jsonData)
        elif radarType == 'ceil':
            pubPayload["TYPE"]="CEIL"
            jsonData = json.dumps(pubPayload)        
            mqttc.publish("/GMT/DEV/" + mac + "/DATA/CEIL/JSON3", jsonData)
        elif radarType == 'vital':
            pubPayload["TYPE"]="VITAL"
            jsonData = json.dumps(pubPayload)        
            mqttc.publish("/GMT/DEV/" + mac + "/DATA/VITAL/JSON3", jsonData)
        # print(jsonData)
        # print("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")              
        time.sleep(0.1)
...
def on_connect(client, userdata, flags, rc):
    ...
    client.publish("/GMT/USVC/DECODE_PUBLISH_DBG3/STATUS","CONNECTED",1,True)
...
mqttc.will_set("/GMT/USVC/DECODE_PUBLISH_DBG3/STATUS","DISCONNECTED",qos=1, retain=True)
mqttc.connect(brokerAddress)
print("Subscribe to topic: "+ "/GMT/DEV/+/DATA/+/RAW/#")
mqttc.subscribe("/GMT/DEV/+/DATA/+/RAW/#")
# print("Subscribe to topic: "+ "/GMT/DEV/F412FAE26208/DATA/+/RAW/#")
# mqttc.subscribe("/GMT/DEV/F412FAE26208/DATA/+/RAW/#")
time.sleep(1)
