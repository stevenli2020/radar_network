...
def on_message(client, obj, msg):
    ...
    if TOPIC[-1] == "UPDATE_DEV_CONF":
        print(
            "Received device setting update request for: " + msg.payload.decode("utf-8")
        )
    ...
    sql = (
        "UPDATE `DEVICES` SET `LAST_DATA_RECEIVED`=NOW(),`STATUS`='CONNECTED' WHERE MAC='"
        + MAC
        + "';"
    )
    cursor.execute(sql)
    ...
    if MAC not in devicesTbl:
        ...
        devicesTbl[MAC] = {}
        sql = (
            "SELECT * FROM ROOMS_DETAILS RIGHT JOIN RL_ROOM_MAC ON ROOMS_DETAILS.ROOM_UUID=RL_ROOM_MAC.ROOM_UUID WHERE MAC='"
            + MAC
            + "';"
        )
        cursor.execute(sql)
    ...
    for D in DATA:
        ...
        if Type == "1" or Type == "2":
            ...
            IN_BED_MOVING = (
                "NULL" if D.get("inBedMoving") == None else str(D.get("inBedMoving"))
            )
            SIGN_OF_LIFE = (
                "NULL" if D.get("signOfLife") == None else str(D.get("signOfLife"))
            )
    ...
    if ROOM_STATUS is not None:
        if ROOM_STATUS in [0]:
            ROOM_STATUS = check_room_empty(
                cursor, table_name, devicesTbl[MAC]["ROOM_UUID"]
            )
        sql = (
            "UPDATE `ROOMS_DETAILS` SET `STATUS`="
            + str(ROOM_STATUS)
            + ",`OCCUPANCY`="
            + str(OBJECT_COUNT)
            + ",`LAST_DATA_TS`=NOW() WHERE ROOM_UUID='"
            + devicesTbl[MAC]["ROOM_UUID"]
            + "';"
        )
    else:
        sql = (
            "UPDATE `ROOMS_DETAILS` SET `OCCUPANCY`="
            + str(OBJECT_COUNT)
            + ",`LAST_DATA_TS`=NOW() WHERE ROOM_UUID='"
            + devicesTbl[MAC]["ROOM_UUID"]
            + "';"
    ...
    client.publish(
        msg.topic.replace("/DATA/", "/EXTRA_DATA/"), json.dumps(PAYLOAD), qos=1
    )
