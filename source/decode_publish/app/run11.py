...
def publishProcessData(clientID1, userName1, userPassword1, processDataQueue, dataBufferQueue):

  mqttc1 = mqtt.Client(clientID1)
  mqttc1.username_pw_set(userName1, password=userPassword1)
  mqttc1.connect(brokerAddress,1883,10)
  mqttc1.loop_forever()
    ...
    try: 
        ...
        if len(pubPayload["DATA"]) > 0:
            ...
            if radarType == 'wall':
                ...
                jsonData = json.dumps(pubPayload)
                print("publishing...")
                result = mqttc1.publish("/GMT/DEV/"+mac+"/DATA/WALL/JSON", jsonData)
                print(result)
            elif radarType == 'ceil':
                pubPayload["TYPE"]="CEIL"
                jsonData = json.dumps(pubPayload)
                print("publishing...")
                result = mqttc1.publish("/GMT/DEV/" + mac + "/DATA/CEIL/JSON", jsonData)
                print(result)
            elif radarType == 'vital':
                pubPayload["TYPE"]="VITAL"
                jsonData = json.dumps(pubPayload)
                print("publishing...")
                result = mqttc1.publish("/GMT/DEV/" + mac + "/DATA/VITAL/JSON", jsonData)
                print(result)
            print("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")              
            time.sleep(0.1)
            # dataBuffer.append(jsonData)
...
if __name__ == '__main__':
    ...
    # _thread.start_new_thread(updataAlgoCfg)
