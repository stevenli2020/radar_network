...
async function init(element, macA=null) {
    // Create div that wroaps all the elements inside (select, elements selected, search div) to put select inside
    const multipleSelect = document.getElementById('mul-sel')
    ...
    await fetch(`${host}/api/getRoomDetails`, {
        ...
        .then((data) => {
          ...
          if (data.DATA) {
            ...
            //   optionTag.setAttribute('value', d.MAC)
            //   optionTag.innerText = d.NAME
              optionTag.setAttribute('value', d.ROOM_NAME)
              optionTag.innerText = d.ROOM_NAME
              if(macA != null){
                for(var i=0; i < macA.length; i++){
                    if(macA[i] == d.ROOM_NAME){
                        optionTag.setAttribute('selected', 'selected')
                    }
                }
              }
              multipleSelect.appendChild(optionTag)
...
function createInitialTokens(select) {
    ...
    for (let i = 0; i < options_selected.length; i++) {
        // console.log(options_selected[i])
...
function openOptions(e) {
    ...
    const dropdown = wrapper.querySelector(".dropdown-icon");
    if (!dropdowcreateTokenn.classList.contains("active")) {
        const event = new Event('click');
        dropdown.dispatchEvent(event);
    }
    e.stopPropagation();
...
function mulSel(macA=null){
    // get select that has the options available
    const select = document.querySelectorAll("[data-multi-select-plugin]");
    select.forEach(select => {
        init(select, macA);
    });
    // const addMulSel = document.getElementById('mul-sel')
    // const updateMulSel = document.getElementById('update-mul-sel')
    // init(addMulSel)
    // init(updateMulSel)
    // Dismiss on outside click
    document.addEventListener('click', () => {
