...
def getSummaryDataofPosition(data):
    ...
    if (len(tables)>0):
        ...
        for table in tables:
            if (t=="CUSTOM"):
                combine_table.append(f"""SELECT * FROM Gaitmetrics.{table} WHERE MAC='{data['DEVICEMAC']}' AND `PX` IS NOT NULL AND PY IS NOT NULL{filtering}""")
            else:
                combine_table.append(f"""SELECT * FROM Gaitmetrics.{table} WHERE MAC='{data['DEVICEMAC']}' AND TIMESTAMP > DATE_SUB(NOW(), INTERVAL {timeRange}) AND `PX` IS NOT NULL AND PY IS NOT NULL{filtering}""")
